...
public final class KitSelectionMenu extends MenuHolder {
    public static final String CERTIFICATION_QUEUE_NAME = "&aCertification Queue";
    ...
    public KitSelectionMenu(Purpose purpose, Collection<Kit> kits, RatingService ratings, UUID viewerId) {
        super(sizeFor(kits.size()), titleFor(purpose));
        ...
        for (Kit kit : kits) {
            ...
            getInventory().setItem(slot++, new ItemBuilder(kit.getIcon())
                    ...
                    .lore(loreFor(purpose, kit, ratings, viewerId))
                    .build());
        }
        ...
        getInventory().setItem(footer, new ItemBuilder(Material.BOOK)
                .name(purpose == Purpose.BOT ? "&bStep 1: Choose a Kit"
                        : purpose == Purpose.QUEUE ? "&bRanked Queue" : "&bKit Layouts")
                .lore(purpose == Purpose.QUEUE && ratings != null ? new String[] {
        ...
        getInventory().setItem(getCloseSlot(), new ItemBuilder(Material.BEDROCK)
                .name("&cClose").lore("&eLeft Click: &fReturn to the lobby").build());
        if (purpose == Purpose.QUEUE) {
            List<String> certificationLore = new ArrayList<String>();
            certificationLore.add("&7Play fixed-bot certification matches.");
            certificationLore.add("&7Complete 3 per kit to unlock Ranked Queue.");
            if (ratings != null && viewerId != null) {
                certificationLore.add("");
                for (Kit kit : kits) {
                    certificationLore.add("&7" + kit.getDisplayName() + ": "
                            + (ratings.isQualified(viewerId, kit.getId()) ? "&aCertified" : "&e"
                                + ratings.getPlacementCount(viewerId, kit.getId()) + "/3"));
                }
            }
            certificationLore.add("");
            certificationLore.add("&eLeft Click: &fSelect a certification kit");
            getInventory().setItem(getCertificationSlot(), new ItemBuilder(Material.EXP_BOTTLE)
                    .name(CERTIFICATION_QUEUE_NAME)
    ...
    static String[] loreFor(Purpose purpose, Kit kit, RatingService ratings, UUID viewerId) {
        ...
        if (qualified) {
            lines.add("&7Your ELO: &b" + RatingService.format(ratings.getRatingMilli(viewerId, kit.getId())));
