...
横幅の基準となる区切り線を20文字から13文字（従来の65%、約66%）へ縮小しています。
表示項目と座標の桁は省略しません。Minecraftのスコアボードは最長行に合わせて幅が決まるため、
極端に大きな人数・Pingなどが表示される場合は必要な幅まで広がります。
...
| `/sworld regenerate <name\|all>` | 再生成の対象を確認してトークンを発行（`regen`でも可） |
| `/sworld regenerate confirm <token>` | 次回起動前の再生成・復元を予約 |
| `/sworld regenerate status` | 現在の予約を確認 |
| `/sworld regenerate cancel` | 未実行の予約を取り消す |
| `/sworld regenerate restore <backup-id>` | 再生成前の地形への復元確認トークンを発行 |
...
**確認コマンドだけでは稼働中のワールドを変更しません。** 初期ワールドを含む全対象で共通して、
次回の正常停止後、起動スクリプトがPaperを起動する前に予約を実行します。コマンドが勝手にサーバーを停止することはありません。

```text
/sworld regenerate survival
/sworld regenerate confirm <表示されたトークン>
/sworld regenerate status
```

`survival` は通常世界のみです。Nether・Endは `/sworld list` の名前か `the_nether`・`the_end` で個別指定できます。
`all` は **初期3ディメンションと、読み込み済みの追加管理ワールドすべて**です。
確認時には対象ディメンションを列挙します。削除済みワールドや管理外ワールドは含みません。
確認は管理者ごとに紐づき、既定30秒で失効します。予約は同時に1件だけで、予約中の作成・削除・削除からの復元も停止します。

- 建築物、チェスト、地形、地面のアイテム、Mob等をリセットし、**元と同じシード・生成設定**で生成し直します。
- プレイヤーの所持品、エンダーチェスト、経験値、実績、統計を保持します。
- ワールドUUID、難易度、ゲームルール、ボーダー等の設定も保持します。Endの旧ドラゴン戦の状態は引き継ぎません。
- 対象ワールドに残っていたプレイヤーは、再接続時に通常世界の安全な地上へ退避します。安全な場所が見つからない場合はログインを拒否して事故を防ぎます。
- Paper 26.3のディメンション内メタデータを使います。共有の`level.dat`・`players`は移動しません。

旧ディメンションは `.ascending-regenerated-worlds/<backup-id>/original/` へ退避し、永久削除しません。
復元は `/sworld regenerate restore <backup-id>` →表示された確認コマンド→正常停止・起動の順です。
復元時にも現在の地形を別のバックアップへ退避し、元のバックアップも保持します。
このコマンドは未復元の再生成バックアップを1回復元するためのものです。復元後の世代の手動復旧はサーバー停止中に運用者が行ってください。

ローカルは必ず更新後の `run.bat`、コンテナは通常の起動処理を使ってください。`java -jar paper.jar` の直接実行は予約処理を迂回するため使用しないでください。
ローカルの既存構成には `scripts/setup-network.ps1 -UpdateExisting` でJARと起動スクリプトを一緒に反映します。
稼働中のワールドに対するファイル操作はセッションロックで拒否します。中断した処理は記録を確認して再開し、
不明確な状態・パス不正・メタデータ不足ではPaperを起動せず停止します。予約やバックアップの管理ファイルを手動削除しないでください。
復元用コピーや複数世代の退避でディスクを消費するため、十分な空き容量と外部バックアップを用意してください。
