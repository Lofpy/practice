...
| 状態 | 表示内容 |
| --- | --- |
| PvPロビー・Queue | `Poppy Practice`、上下の区切り線、3KitのELOまたは `Tier n/3`、左寄せの `Online` |
...
認定はHard相当の戦闘値を固定し、移動だけ次の専用値・制御を使用します。
...
| 有効命中後 | 8 tickは横入力0、1 tickのW離しと間合い維持 |
...
Boxingは回復・パールを使用しません。Comboは回復スプラッシュ設定を使わず、支給された上位金リンゴ・金ニンジン・予備防具を実際に消費します。
