...
entries = [x for x in entries if x["name"] in
           ("velocity.jar", "paper.jar", "ViaVersion.jar", "ViaBackwards.jar", "ViaRewind.jar")]
assert {x["name"] for x in entries} == {
    "velocity.jar", "paper.jar", "ViaVersion.jar", "ViaBackwards.jar", "ViaRewind.jar"
}, "All pinned server artifacts must be present"
entries.append({
    "name": "windspigot.jar",
...
for entry in entries:
    request = urllib.request.Request(entry["url"], headers={
        "User-Agent": "AscendingMC-CI/1.0 (https://github.com/Lofpy/practice)"})
